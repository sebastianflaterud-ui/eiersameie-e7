...
-- Add new columns to leietakere
ALTER TABLE leietakere ADD COLUMN IF NOT EXISTS konto_id text;
ALTER TABLE leietakere ADD COLUMN IF NOT EXISTS forfall_dag integer DEFAULT 1;

-- Add new columns to leieforhold
ALTER TABLE leieforhold ADD COLUMN IF NOT EXISTS kontrakt_id uuid REFERENCES kontrakter(id);
ALTER TABLE leieforhold ADD COLUMN IF NOT EXISTS forfall_dag integer DEFAULT 1;

-- Create betalingsmottakere table
CREATE TABLE IF NOT EXISTS betalingsmottakere (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id uuid NOT NULL,
  leieforhold_id uuid NOT NULL REFERENCES leieforhold(id) ON DELETE CASCADE,
...
ALTER TABLE betalingsmottakere ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Users can view own betalingsmottakere" ON betalingsmottakere FOR SELECT USING (auth.uid() = user_id);
CREATE POLICY "Users can insert own betalingsmottakere" ON betalingsmottakere FOR INSERT WITH CHECK (auth.uid() = user_id);
CREATE POLICY "Users can update own betalingsmottakere" ON betalingsmottakere FOR UPDATE USING (auth.uid() = user_id);
CREATE POLICY "Users can delete own betalingsmottakere" ON betalingsmottakere FOR DELETE USING (auth.uid() = user_id);

-- Update seed function with new tenant data and leieforhold
