...
interface Enhet { id: string; navn: string; status: string; }
...
export default function Beleggsoversikt() {
  ...
  const load = useCallback(async () => {
    const [e, lf, lt, tx] = await Promise.all([
      supabase.from('enheter').select('id, navn, status').eq('aktiv', true).order('navn'),
  ...
  // For each enhet+month, find who's occupying based on leieforhold dates
  ...
  // Actual income per month from transactions
  ...
  const expectedPerMonth = Array.from({ length: 12 }, (_, i) => {
    const m = i + 1;
    return enheter.reduce((s, e) => {
      const occ = getOccupant(e.id, m);
      return s + (occ ? occ.leie : 0);
    }, 0);
  });

  // Count occupied per month
  const occupiedPerMonth = Array.from({ length: 12 }, (_, i) => {
    const m = i + 1;
    return enheter.filter(e => getOccupant(e.id, m) !== null).length;
  });

  const totalActual = actualPerMonth.reduce((s, v) => s + v, 0);
  const avgOccupancy = enheter.length > 0 ? occupiedPerMonth.reduce((s, v) => s + v, 0) / 12 : 0;
  const avgOccPct = enheter.length > 0 ? (avgOccupancy / enheter.length) * 100 : 0;
  const ledigeEnhetsmnd = occupiedPerMonth.reduce((s, v) => s + (enheter.length - v), 0);
    ...
    <div className="space-y-6">
      ...
      <div className="border rounded-lg overflow-auto">
        <Table>
          <TableHeader>
            <TableRow>
              <TableHead className="sticky left-0 bg-background min-w-[120px]">Enhet</TableHead>
          ...
          <TableBody>
            {enheter.map(e => (
              <TableRow key={e.id}>
                <TableCell className="sticky left-0 bg-background font-medium">{e.navn}</TableCell>
            ...
            <TableRow className="border-t-2 font-medium">
              <TableCell className="sticky left-0 bg-background">Antall utleid</TableCell>
              {occupiedPerMonth.map((v, i) => (
                <TableCell key={i} className="text-center">{v} / {enheter.length}</TableCell>
              ))}
            </TableRow>
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
        <Card><CardContent className="pt-4"><div className="text-xl font-bold">{avgOccupancy.toFixed(1)} av {enheter.length}</div><div className="text-sm text-muted-foreground">Gj.sn. belegg</div></CardContent></Card>
